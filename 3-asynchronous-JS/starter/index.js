...
//* Write a special version of promise all
    ...
    promises.forEach((promise) => {
      promise
        ...
        .then((result) => {
          counter++;
          results.push(result);
          if (counter === promises.length) return res(results);
        });
    });
  });
};
...
const getDogPic = async () => {
  try {
    ...
    const res1Pro = superagent.get(
      `https://dog.ceo/api/breed/${data}/images/randommm`
    );
    ...
    //* catch all the promises: New method
    // The problem with the way this was solved previously is that
    //
    const all = await myPromiseAll([res1Pro, res2Pro, res3Pro]);
